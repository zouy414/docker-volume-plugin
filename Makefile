...
plugin: image #@help Create docker plugin
	docker plugin disable -f $(EXECUTABLE):$(TAG) || echo "plugin already disabled"
	docker plugin rm -f $(EXECUTABLE):$(TAG) || echo "plugin already removed"
	rm -rf bin/plugin
	mkdir -p bin/plugin/rootfs
	docker export $(shell sudo docker create $(PLUGIN):$(TAG) --name plugin_$(PLUGIN)) | tar -x -C bin/plugin/rootfs
	cp cmd/docker-volume-plugin/config.json bin/plugin
	sudo docker rm -vf plugin_$(PLUGIN)
	sudo docker rmi -f $(PLUGIN):$(TAG)
	sudo docker plugin create $(PLUGIN):$(TAG) bin/plugin
